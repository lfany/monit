...
If no existance test is defined, the implicit non-existence test with restart
...
This test will trigger the action if the object (for example process or filesystem),
doesn't exist. It can be used for example to make sure apache is running, data filesystem
is mounted, etc.
...
This test is opossite if the non-existence test: it will trigger the action if the object
