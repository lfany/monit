...
B<-B>
   Batch command line mode (nontabular output with no colors)
...
L<status|/"PROGRAM STATUS TESTING"> test allows one to check the
...
made available in the User Interface and in alerts, by default up to
512B. You can customize the limits using the L<set limits|"LIMITS">
statement).
...
C<-l> command-line switch (or the keyword I<set logfile syslog>
...
can use either -B option or the following statement in the monit
configuration file to disable the tabular output and colors
completely:
...
this option to allow self-signed certificates (not recommended for
production).

I<CIPHERS> override the default SSL/TLS ciphers. The default settings
is "ALL:!DES:!RC4:!aNULL:!LOW:!EXP:!IDEA:!MD5:@STRENGTH"
...
I<CLIENTPEMFILE> set the path to the SSL client certificate
"database-file" in PEM format. If an SSL server requires client
certificate authentication, Monit will try to find a public key
certificate in this file which match the server's Certificate Authority
and if found, use this certificate for client authentication.
...
status") need to communicate with the Monit background process via
...
by default (if ADDRESS option is missing):
...
connections over IP version 6, specify a IPv6 address:
...
over IP version 4, specify a IPv4 address:
...
I<PEMFILE> option sets path to the PEM formatted file, which contains the
server's private key and certificate. This file should be stored in a safe
place on the filesystem and should have strict permissions, no more than 0700.
...
B<CLIENTPEMFILE> option enables the client certificate based authentication.
A connecting client has to provide a certificate known to Monit in order
to connect.  This file also needs to have all necessary CA certificates.
By default self-signed client certificates are B<not> allowed. If you want
to use a self signed certificate from a client it has to be allowed explicitly
with the B<SELFSIGNED: ALLOW> option.
...
by Monit. The client must connect to Monit over SSL and Monit ask the
client to send its certificate. Upon receiving the certificate Monit
compares the certificate to certificates located in the I<CLIENTPEMFILE>
SSL option. Access is granted if the client certificate is in this file.
See L<SSL|"SSL settings"> settings for details.
...
And monitrc config which only allows group C<admin> authenticated via
...
Alternatively you can use files in C<htpasswd> format (one
I<user:passwd> entry per line), like so: I<allow [cleartext|crypt|md5]
/path [users]>. By default cleartext passwords are read. In case the
...
This test allows to trigger an action based on the monitored object existence.
It is supported for I<process>, I<file>, I<directory>, I<filesystem> and I<fifo>
services.

If no existence test is defined, the implicit non-existence test with restart
action is activated, so for example if the process stops, Monit will restart it.

There are two object existence tests:

=head3 OBJECT NON-EXISTENCE

This test will trigger the action if the object doesn't exist. It can be used for
example to make sure apache is running, data filesystem is mounted, etc.

 IF [DOES] NOT EXIST THEN <action>

I<action> is a choice of "ALERT", "RESTART", "START", "STOP", "EXEC" or "UNMONITOR".

Example: exec script if the filesystem does NOT exist:
...
=head3 OBJECT EXISTENCE

This test is opossite of the non-existence test: it will trigger the action if the object
DOES exist. It can be used for example to kill the process which shouldn't be running.

 IF [DOES] EXIST THEN <action>

I<action> is a choice of "ALERT", "RESTART", "START", "STOP", "EXEC" or "UNMONITOR".

Example: kill the process if it was started:

 check process matching "EvilProcess"
       if does exist then exec "/usr/bin/pkill -9 EvilProcess"
...
"CPU([user|system|wait])", "MEMORY", "SWAP", "THREADS", "CHILDREN",
"TOTAL MEMORY", "LOADAVG([1min|5min|15min])". Some resource tests can
be used inside a check system entry, some in a check process entry and
some in both:
...
optional, if not used, the total system cpu usage is tested.
...
CPU is the CPU usage of the process itself (percent). Monit calculates
...
I<value> is either an integer or a real number. For CPU, TOTAL CPU,
MEMORY and TOTAL MEMORY you need to specify a I<unit>. This could
be "%" or if applicable "B" (Byte), "kB" (1024 Byte), "MB" (1024 KiloByte)
or "GB" (1024 MegaByte).
...
=head2 PROCESS DISK I/O TESTING

Monit can test process' filesystem read and write activity. This test
may only be used in the context of a process service type.

The platform usually supports the per-process I/O activity statistics
either by bytes or by operations.
...
=head2 FILESYSTEM MOUNT FLAGS TESTING
...
=head2 DISK I/O TESTING

Monit can test a filesystem read and write activity. This test may
only be used in the context of a filesystem service type.

The I/O granularity and supported filesystems depend on the platform
API. Some platforms allow to get I/O activity for specific pertition,
other just for the whole disk. Some allow to get statistics for
network filesystems too, others just for block devices.

Platforms I/O statistics granularity and filesystem support:

 ---------------------------------------------------------------------------------------
 | Platform     | Granularity    | Supported filesystems                      | TBD    |
 ---------------------------------------------------------------------------------------
 | AIX          | per-disk       | disk io monitoring not supported currently | JFSx   |
 | DragonFlyBSD | per-disk       | UFS                                        | HAMMER |
 | FreeBSD      | per-disk       | UFS                                        | ZFS    |
 | Linux        | per-filesystem | EXTx, XFS, BTRFS, NFS, CIFS                |        |
 | MacOS        | per-disk       | HFS                                        |        |
...
Service time is IO transaction completion time minus IO transaction
start time. For example, if you read data, it is the time between
requesting data and when you get the data. Service time is a fairly
important metric. If it grows, it means that the disk is not able to
handle the operations fast enough.
...
=head2 NETWORK LINK STATUS TEST
...
=head2 NETWORK LINK CAPACITY TEST
...
The B<SIZE> parameter specifies the ping request data size. Default
is 64 bytes.
...
M/Monit uses Monit as an agent. With regular intervals, Monit can send
a message to M/Monit with the status of the host it is running on.
...
mobile devices. From M/Monit, you can also start, stop and restart
services on your hosts running Monit.
...
The default timeout is 5 seconds, you can customise the timeout
using the I<TIMEOUT> option.

When Monit registers itself in M/Monit, it sends by default credentials that
can be used to perform service actions from M/Monit. You can disable credentials
sending by I<REGISTER WITHOUT CREDENTIALS> option.
...
check to see if our web server is running and log a message if not:
...
cycles or the memory usage is over 100Mb for five cycles or if the
machines load average is more than 10 for 8 cycles:
...
Copyright (C) 2001-2017 by Tildeslash Ltd. All Rights Reserved.
