...
I<PEMFILE> set the path to the PEM formatted file, which contains the
...
B<CLIENTPEMFILE> enables client certificate authentication. A
connecting client has to provide a certificate known to Monit in order
to connect. This file also needs to have all necessary CA certificates.
By default self-signed client certificates are B<not> allowed. If you
want to use a self signed certificate from a client it has to be
allowed explicitly with the B<SELFSIGNED: ALLOW> option.
...
Relative timestamp syntax:
 ...
 IF [ACCESS|MODIFICATION|CHANGE] TIMESTAMP [[operator] value [unit]] THEN action

Timestamp change syntax:

 IF CHANGED [ACCESS|MODIFICATION|CHANGE] TIMESTAMP THEN action
...
Filesystem allow to disable I<atime> updates using mount options, so
this test will work if the filesystem performs atime updates.
...
=item DEFAULT (LATEST OF CHANGE AND MODIFICATION TIMESTAMPS)
...
Example for sending alert if log file is not updated for more than 1 hour:
...
the HTTP HEAD request method to save bandwidth, unless a response content
or response checksum is tested. As some webservers may not support the
HEAD method, one may want to set the method explicitly.
