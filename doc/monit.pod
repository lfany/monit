...
be executed and if the program requires arguments you must enclose
the program and its arguments in a quoted string. You may
optionally specify the uid and gid the executed program should
switch to upon start. The program is by default executed only once,
on the state change. You can enable program repetition when the error
persists for given number of cycles. For instance:
...
This test is implicit and always active for service checks of type,
I<process>, I<file>, I<directory>, I<filesystem> and I<fifo>. If not
defined, it defaults to a restart action.
...
Example:
        ...
        if disk write rate > 1MB/s then alert
...
Platforms I/O statistics granularity and filesystem support:
 ...
 | Linux        | per-filesystem | EXTx, XFS, NFS, CIFS                       |        |
 | MacOS        | per-disk       | HFS                                        |        |
...
Example:
        ...
        if write rate > 1MB/s then alert
