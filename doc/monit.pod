...
512B, you can customize the limit using the L<set limits|"LIMITS">
statement).
...
this option to allow self-signed certificates.
...
B<SSL> enable TLS for Monit's web interface. The I<PEMFILE> option
...
B<CLIENTPEMFILE> client certificate based authentication. A connecting
client has to provide a certificate known to Monit in order to connect.
This file also needs to have all necessary CA certificates. By default
self-signed client certificates are B<not> allowed. If you want to use
a self signed certificate from a client it has to be allowed explicitly
with the B<ALLOWSELFCERTIFICATION> statement.
...
transparent and does not require any special configuration. If
...
Monit will bind to and listen on port 2812 on all interfaces, both IPv6
and IPv4 interfaces if available. To force Monit HTTP to only listen on
and accept connections over IP version 6, specify an IPv6 address
...
in this file. See I<CLIENTPEMFILE> above for details.
...
=head1 CENTRAL MANAGEMENT OF MULTIPLE MONIT HOSTS WITH M/MONIT

You can use L<M/Monit|https://mmonit.com> for central management of multiple
Monit hosts and to keep event log, draw charts, trends and more.

To send data to M/Monit, add the following statement:
...
Monit will register itself in M/Monit and will start sending service
statistics (for charts) and events to M/Monit.

By default Monit registers its own credentials from the SET HTTPD statement's
ALLOW option, so M/Monit can communicate back to Monit (such as restart a specific service).

It is possible to disable credentials registration, Monit will still be able
to send statistics and events to M/Monit, but if you'll want to control some
service remotely from M/Monit, you'll have to enter credentials manually. Example:

 set mmonit http://monit:monit@192.168.1.10:8080/collector and register without credentials

The password should be URL encoded if it contains URL-significant characters like ":", "?", "@".

Important: always use M/Monit with HTTPS url scheme, so the communication is encrypted.
...
You can override this timeout using the I<TIMEOUT> option.
...
This test is implicite and always active for service checks of type,
I<process>, I<file>, I<directory> and I<fifo>. If not defined, it
defaults to a restart action.
...
I<localaddress: ADDRESS string>. The source IP address to use.

I<ipversion: IPV4 | IPV6 >. Optionally specify the IP version Monit
...
I<type: TYPE [TCP | UDP]>. Optionally specify the socket type Monit
...
I<ssl: [SSL | TLS] [with options {...}]>. Set SSL/TLS L<options|"SSL
...
I<certificate checksum: CERTIFICATE CHECKSUM [MD5|SHA1] hash>. Verify
...
I<certificate expire days: CERTIFICATE VALID for number DAYS>. Send
...
I<protocol: PROTO(COL) protocol>. Optionally specify the protocol Monit
...
Copyright (C) 2001-2016 by Tildeslash Ltd. All Rights Reserved.
